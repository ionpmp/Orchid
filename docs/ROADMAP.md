...
- [~] Document editor (DOCX) — Tier-1 rich text model + OOXML read/write (fonts/bold/italic/underline/color, alignment, lists via `numbering.xml`/`numId`, simple tables, page setup); `DocumentViewer` + dispatch before ZIP-archive; Slint document surface (`kind=7`) with toolbar + Preview/Source toggle; parley+swash RGBA preview canvas; selection-scoped B/I/U (run split) + align/list on selected paragraphs; viewport-driven preview width; plain `TextInput` source mode; undo/redo; Tantivy DOCX extractor; Word binary fixtures still pending
