...
- [~] Widget: Audio player — local library (Songs/Artists/Albums/Folders), queue, shuffle/repeat, sleep timer, favorites + playlists, EQ presets, playback speed, soft volume boost (to 150%), library search, gapless prefetch, sidecar `.lrc` lyrics, background library scan, Windows SMTC; shared audio-only libmpv session
- [x] Widget: RSS feed
