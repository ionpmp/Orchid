...
- [~] Widget: Audio player — local library (Songs/Artists/Albums/Folders/Genres), queue (add / play-next / reorder / clear / jump-to-current / reshuffle remaining; list shows shuffle play order), shuffle/repeat, soft crossfade (3–12s), sleep timer, favorites + playlists (rename/delete/add/remove) + Recent (last 50 played), EQ presets, ReplayGain, playback speed, soft volume boost (to 150%), library search (play/enqueue all matches), library sort (Artist/Title/Album/Year/Genre), gapless prefetch, sidecar `.lrc` lyrics, background library scan, focused keyboard transport (1–7 browse tabs, G show folder, C copy path, Escape clear search/filter), Windows SMTC; play/enqueue browse group; library root chips + remove; Explorer drop (folders→roots, audio→queue); session restore (queue track loaded paused); browse breadcrumb + album line + rename prefill; persisted EQ/RG/speed; FM Play/Enqueue in Audio Player + show track folder; M3U export/import (queue/playlists); track duration from ID3 TLEN + cover thumbs in list rows; widget-owned volume (not Viewer `media_prefs`); mutual pause with Viewer media; shared audio-only libmpv session
