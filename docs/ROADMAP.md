...
- [~] Internationalization (11 languages, RTL) — 11 Fluent catalogues bundled (`en-US`…`ar-SA`); widget titles + catalog/dock descriptions, FM/viewer sizes, System uptime/battery charging·time, Properties/Details/delete confirm, search empty states + open hint, startup status strip (theme display name + locale endonyms), FM Home/loading/empty/access + encrypted/managed entry hints + OS I/O error mapping + elided filename hover, settings shortcuts (resolved command names)/coming-soon/disabled/default placeholders + section sidebar hover, TOTP remaining, PDF page-of/Go, archive extract/parent hints, weather/search/onboarding/dialog hover chrome, viewer unsupported type + text LF/CRLF/syntax/encoding + image/archive format labels + localized viewer/password/search error fallthrough, recent-files/RSS open hints, command palette `orc` invocation, terminal/password (list + Copy/Open)/catalog/dock/FM/workspace/group chrome hover hints + widget resize tooltip localized; S-size RTL mirrors notification/workspace docking when language starts with `ar`
