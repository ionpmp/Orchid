...
- [~] Document editor (DOCX) — Tier-1 rich text model + OOXML read/write (fonts/bold/italic/underline/color, alignment, lists, simple tables, page setup); `DocumentViewer` + dispatch before ZIP-archive; Slint document surface (`kind=7`) with toolbar + Preview/Source toggle; parley+swash RGBA preview canvas; plain `TextInput` source mode; undo/redo; Tantivy DOCX extractor; selection-scoped formatting and Word round-trip fixtures still pending
