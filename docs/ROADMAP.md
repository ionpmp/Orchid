...
- [~] Internationalization (11 languages, RTL) — 11 Fluent catalogues bundled (`en-US`…`ar-SA`); widget titles, FM/viewer sizes, System uptime, Properties/Details/delete confirm, search empty states, startup status strip, FM loading, settings coming-soon/open-in-editor, TOTP remaining, terminal/password/catalog/workspace hover hints localized; S-size RTL mirrors notification/workspace docking when language starts with `ar`
