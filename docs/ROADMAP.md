...
- [~] Document editor (DOCX) — Tier-1 rich text model + OOXML read/write (fonts/bold/italic/underline/color, alignment, lists, simple tables, page setup); `DocumentViewer` + dispatch before ZIP-archive; Slint document surface (`kind=7`) with toolbar (save/undo/redo/B/I/U/align/lists) + plain `TextInput`; undo/redo; Tantivy DOCX extractor; parley→Slint rich canvas and Word round-trip fixtures still pending
