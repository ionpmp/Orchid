...
- [~] Internationalization (11 languages, RTL) — 11 Fluent catalogues bundled (`en-US`…`ar-SA`); widget titles, FM/viewer sizes, System uptime, Properties/Details/delete confirm, search empty states, startup status strip, FM loading/empty/access errors, settings coming-soon/disabled/default placeholders, TOTP remaining, PDF page-of/weather status/search source/onboarding step, viewer unsupported type, terminal/password/catalog/dock/FM chrome hover hints localized; S-size RTL mirrors notification/workspace docking when language starts with `ar`
