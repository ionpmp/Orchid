...
- [~] Media (audio/video) — libmpv in-app playback (RGBA blit) with play/pause/seek/volume/speed/mute/fullscreen, folder playlist, audio tracks + chapters, album cover (APIC / folder cover), A-B loop + resume position, embedded + sidecar `.srt`/`.ass` subs; falls back to system player (`opener`) when DLL missing; ID3 tag report in FM Tools
