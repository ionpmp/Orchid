...
- [~] Document editor (DOCX) — Tier-1 rich text model + OOXML read/write (fonts/bold/italic/underline/color, alignment, lists via `numbering.xml`/`numId`, simple tables, page setup); `DocumentViewer` + dispatch before ZIP-archive; Slint document surface (`kind=7`) with toolbar + Preview/Source toggle; parley+swash RGBA preview canvas with click/drag/double-click-word selection, caret paint, keyboard typing (insert/delete/return/arrows/word-jump/Home/End/Ctrl+A), and clipboard copy/cut/paste; selection-scoped B/I/U + font size/family/color + align/list (Tab/Shift+Tab indent) on selected paragraphs; viewport-driven preview width; plain `TextInput` source mode; undo/redo; Tantivy DOCX extractor; Word/LibreOffice fixtures under `tests/fixtures/docx/` + parse/round-trip tests
