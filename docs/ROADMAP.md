...
- [x] Widget: Audio player — local library (Songs/Artists/Albums/Folders/Genres), queue (add / play-next / reorder / clear / jump-to-current / reshuffle remaining; list shows shuffle play order; auto-scroll to current; remaining tracks/time strip), shuffle/repeat, soft crossfade (3–12s), sleep timer, favorites + playlists (rename/delete/add/remove) + Recent (last 50 played), EQ presets, ReplayGain, playback speed, soft volume boost (to 150%), library search (play/enqueue all matches), library sort (Artist/Title/Album/Year/Genre), gapless prefetch, scrollable lyrics panel (sidecar `.lrc`, ID3 `SYLT`/`USLT`, Vorbis/FLAC comments; click-to-seek), libmpv duration probe when TLEN missing, background library scan, focused keyboard transport (1–7 browse tabs, G show folder, C copy path, Escape clear search/filter), Windows SMTC; play/enqueue browse group; library root chips + remove; Explorer drop (folders→roots, audio→queue); session restore (queue track loaded paused); browse breadcrumb + album line + rename prefill; persisted EQ/RG/speed/lyrics panel; FM Play/Enqueue in Audio Player + show track folder; M3U export/import (queue/playlists); track duration from ID3 TLEN + cover thumbs in list rows; widget-owned volume (not Viewer `media_prefs`); mutual pause with Viewer media; shared audio-only libmpv session
- [~] Widget: Video player — local video library (folders + queue), shuffle/repeat, speed, volume, libmpv RGBA blit; FM Play/Enqueue; Explorer drop; catalog dock widget (distinct from Media Player viewer launcher)
- [x] Widget: RSS feed
...
- [x] PDF (pdfium) — outline sidebar, text layer, find, drag-select/copy, print, highlight export (sibling `hl.pdf`); page navigation (toolbar + PageUp/Down/←/→), go-to-page, fit width/page, zoom, hover hints, viewport re-fit; form fill / signatures later; requires bundled `pdfium.dll`
...
- [~] Media (audio/video) — libmpv in-app playback (RGBA blit ≤1920×1080, viewport-adaptive) with play/pause/seek/stop/volume/speed/mute/fullscreen, folder playlist side panel (Q) + shuffle + loop + EOF auto-next + Home/End, frame step + screenshot PNG, sleep timer (T), aspect/rotate/audio-delay, wheel seek + double-click fullscreen, Explorer drop-open, audio tracks + chapters, album cover (APIC / folder cover), A-B loop + resume, manual sub-add + scale/pos, simple EQ presets, `hwdec=auto-copy` (H toggles SW, persisted), persisted volume/mute, OSD flash, Windows SMTC publish, video chrome auto-hide, pitch-preserving speed (K), pause idle CPU trim, remember last picker folder, sub style presets (G), ReplayGain (U), media kiosk/Esc/next-monitor, persisted look + playlist panel, blit inset for playlist, chapters popup, volume scrubber; catalog **Media Player** launcher (file picker → canvas viewer); dedicated **Video Player** widget (library folders + queue, distinct from viewer and from **Now Playing** SMTC); embedded + sidecar `.srt`/`.ass` subs; falls back to system player (`opener`) when DLL missing; ID3 tag report in FM Tools
- [~] HTML — source preview + Open in the system browser (no embedded WebView yet)
- [~] Document editor (DOCX) — Tier-1 rich text model + OOXML read/write (fonts/bold/italic/underline/strikethrough/double-strikethrough (`w:dstrike`)/highlight/superscript/subscript/all-caps (`w:caps`)/small-caps (`w:smallCaps`)/hidden (`w:vanish`)/shadow (`w:shadow`)/emboss (`w:emboss`)/imprint (`w:imprint`)/outline level (`w:outlineLvl`)/named paragraph styles from `styles.xml` (`w:pStyle` round-trip; Ol cycle sets `Heading1`…`Heading9`; Preview merges style run props + style `w:pPr` spacing/indent/align/shade/borders)/named character styles (`w:rStyle` + `styles.xml` type=character round-trip; Preview merges under direct formatting; Ch cycles on selection)/comments (`word/comments.xml` + range markers round-trip; Cm inserts / Cx deletes / Ce edits text / Cn·Cp jumps next·prev at caret; Preview amber wash; status comment count + caret preview)/color, external `w:hyperlink` + rels round-trip (preview blue+underline; Ctrl+click opens http/https/mailto; hover pointer; insert/edit/remove via Link toolbar / Ctrl+K; internal #bookmark via w:anchor + w:bookmarkStart, Ctrl+click jumps; Bm toolbar inserts bookmark at caret), alignment, lists via `numbering.xml`/`numId` (preserve package numbering when numIds still resolve; Orchid abstracts cover `ilvl` 0..=8), simple tables with cell-paragraph cursor/typing (Enter split / Backspace join, align/list/indent, Tab/Shift+Tab cell nav, insert table / insert/delete row/col via toolbar), merge/unmerge cells (toolbar M+/M−; horizontal prefer, else vertical), `gridSpan`/`vMerge` OOXML round-trip on cells (preview spans columns/rows; continue slots covered by restart), column grid preview from `tblGrid`/`tcW` (equal-width when absent) + hairline borders + column-aware hit-test, cell shading (`w:tcPr/w:shd` fill + preview; Shd toggles cell when caret in table), table cell borders (`w:tcBorders` + preview; Bdr toggles cell box when caret in table), `w:drawing` inside cells → preview blit + click/delete/arrow cell-image cursor, page setup (preview honors `w:pgMar`; Mar+/- margins (caret section); Pg shows/toggles Ltr/A4 (caret section); default header/footer rIds + story paragraphs round-trip (`w:headerReference`/`w:footerReference`, `word/header*.xml`/`footer*.xml`; Preview paints margin stories; save allocates missing parts; Hdr/Ftr toolbar edits plain-text stories (preserves PAGE/DATE/… fields + run/para props); Hf/Ff edit first-page stories; He/Fe edit even-page stories; Preview paints first/even/default header/footer per page-break band; `w:pgMar` `@w:header`/`@w:footer` distances round-trip and Preview honors them; `PAGE`/`NUMPAGES` fields in stories (`w:fldSimple` + complex `w:fldChar` collapse; Preview substitutes per page-break band; Pg# inserts into footer; H±/F± nudge `w:pgMar` header/footer distances (caret section); mid-body section breaks via `w:pPr/w:sectPr` round-trip + Sec toolbar (inserts next-page / cycles continuous `w:type` on existing end; stamps caret-section page setup) / Preview page band; Preview uses per-section header/footer distances + titlePg/evenOdd + body margin x0/wrap + per-section pgSz content width; `DATE`/`FILENAME` fields (`w:fldSimple` + complex collapse; Date/Name toolbar; Preview resolves)); first-page `w:titlePg` + `w:type="first"` header/footer stories round-trip; Tp toolbar toggles `w:titlePg` (caret section); even/odd `w:evenAndOddHeaders` + `w:type="even"` header/footer stories round-trip; Ee toolbar toggles `w:evenAndOddHeaders`)), body inline `w:drawing` → `Block::Image` with OOXML write + media/rels on save; insert via toolbar **Img** / Ctrl+V clipboard image); `DocumentViewer` + dispatch before ZIP-archive; Slint document surface (`kind=7`) with toolbar + Preview/Source toggle; parley+swash RGBA preview canvas with click/drag/double-click-word/triple-click-paragraph selection, caret paint, keyboard typing (insert/delete/Enter paragraph / Shift+Enter soft `w:br` / arrows/word-jump/Home/End/Ctrl+A), clipboard copy/cut/paste (text + image), and inline-image blit; selection-scoped B/I/U/S/H/x²/x₂ + clear formatting (Tx / Ctrl+Space) + font size/family/color + align/list (Tab/Shift+Tab indent) on selected paragraphs; viewport-driven preview width; plain `TextInput` source mode; undo/redo; print (toolbar / Ctrl+P, plain-text shell Print like text viewer); page break (Ctrl+Enter / w:pageBreakBefore, preview rule); keep-with-next (`w:keepNext`; Kn toolbar toggle); keep lines together (`w:keepLines`; Kl toolbar toggle), widow/orphan control (`w:widowControl`; Wc toolbar toggle), contextual spacing (`w:contextualSpacing`; Cs toolbar toggle), RTL paragraph (`w:bidi`; Bi toolbar toggle; Preview mirrors left/right align), suppress auto hyphenation (`w:suppressAutoHyphens`; Hy toolbar toggle); paragraph spacing before/after (w:spacing + Sb±/Sp± toolbar); line spacing (w:line auto/exact/atLeast; Ln± presets for auto); paragraph indent (w:ind left/right/firstLine/hanging; Ind± left, Ir± right, Fl± first/hanging); IME composition via TextInput catcher (preedit strip); status strip word/char counts; catalog **Document** launcher writes a sample `.docx` under `data/documents/` and opens a viewer docked on the canvas (floating chrome has dock ▣; FM/`open_in_viewer` stays floating); in-document Find/Replace (Ctrl+F / F3; Aa match-case) with next/prev over plain text, `n/m` match status, replace current/all, and Source mode caret sync, Preview scroll-to-match; Find/Link hover tips; format/align/list + image/table + font/Preview + Save/Print + spacing/indent/page toolbar tips; Preview zoom (Z± / Ctrl+wheel / Ctrl+0, 50–300%); HiDPI 2× preview raster; paragraph shading (`w:shd` fill + preview; Shd toggle) and paragraph box border (`w:pBdr` all sides + preview; Bdr toggle); Tantivy DOCX extractor; Word/LibreOffice fixtures under `tests/fixtures/docx/` + parse/round-trip tests
