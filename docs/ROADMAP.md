...
- [~] Document editor (DOCX) — Tier-1 rich text model + OOXML read/write (fonts/bold/italic/underline/color, alignment, lists via `numbering.xml`/`numId`, simple tables with cell-paragraph cursor/typing (Enter split / Backspace join, align/list/indent, Tab/Shift+Tab cell nav, insert/delete row/col via toolbar), equal-width column grid preview + hairline borders + column-aware hit-test, `w:drawing` inside cells → preview blit (not cursor-addressable), page setup, body inline `w:drawing` → `Block::Image`); `DocumentViewer` + dispatch before ZIP-archive; Slint document surface (`kind=7`) with toolbar + Preview/Source toggle; parley+swash RGBA preview canvas with click/drag/double-click-word/triple-click-paragraph selection, caret paint, keyboard typing (insert/delete/return/arrows/word-jump/Home/End/Ctrl+A), clipboard copy/cut/paste, and inline-image blit; selection-scoped B/I/U + font size/family/color + align/list (Tab/Shift+Tab indent) on selected paragraphs; viewport-driven preview width; plain `TextInput` source mode; undo/redo; catalog **Document** launcher writes a sample `.docx` under `data/documents/` and opens the floating viewer; in-document Find (Ctrl+F / F3) with next/prev over plain text; Tantivy DOCX extractor; Word/LibreOffice fixtures under `tests/fixtures/docx/` + parse/round-trip tests
