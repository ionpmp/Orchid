...
- [~] Internationalization (11 languages, RTL) — 11 Fluent catalogues bundled (`en-US`…`ar-SA`); widget titles + catalog/dock descriptions (label+desc hover), FM/viewer sizes, System uptime/battery charging·time, Properties/Details/delete confirm, search empty states + candidate/title hover, startup status strip (theme display name + locale endonyms), FM Home/loading/empty/access + encrypted/managed entry hints + OS I/O error mapping + elided filename hover + disabled back/forward + sort-header hints + text-only pane errors, settings shortcuts (resolved command names)/coming-soon/disabled/default placeholders + section sidebar hover, TOTP remaining, PDF page-of/Go, archive extract/parent/path/entry hover hints, weather/search/onboarding/dialog hover chrome, viewer unsupported type + text LF/CRLF/syntax/encoding + image/archive format labels + localized loading/error status (no glyph icons) + viewer/password/search error fallthrough, recent-files path + RSS title/source hover, media track metadata hover, command palette `orc` invocation, terminal/password (list + Copy/Open)/catalog/dock/FM/workspace/group chrome hover hints + widget title elide/resize tooltip localized; S-size RTL mirrors notification/workspace docking when language starts with `ar`
