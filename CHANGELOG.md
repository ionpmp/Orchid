...
- **Tier-1 DOCX document editor**: OOXML read/write, Preview/Source,
  ...
  (Link toolbar / Ctrl+K; internal #bookmark), print (toolbar / Ctrl+P), page breaks (Ctrl+Enter), paragraph spacing (Sp±), line spacing auto (Ln±), IME composition (preview), page margins from `pgMar` in preview (Mar+/-), status word/char counts, find match-case (Aa), catalog **Document** launcher,
