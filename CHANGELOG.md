...
- Spec + Phase 1–3 for native **`.orchid`** container (`crates/orchid-format`:
  ...
  repack, Version-history stub, C2PA Provenance via signed PNG carrier validated
  by the `c2pa` Reader, CLI create/read with `--passphrase` / `--sign-c2pa` /
  `--verify-c2pa`) — see [`docs/ORCHID_FORMAT.md`](docs/ORCHID_FORMAT.md).
