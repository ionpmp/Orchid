...
impl MainWindowController {
    ...
    pub(super) fn build_widget_frame_for_placed(
        ...
        let empties = EmptyWidgetPack::cached(&self.locale);
        ...
        let (
            ...
            match &ws.payload {
                ...
                WidgetPayload::AudioPlayer(a) => (
                    ...
                    empties.search.clone(),
                    empties.media.clone(),
                    build_audio_player_model(a, &self.locale),
                    empties.video_player.clone(),
                    empties.password.clone(),
                    empties.viewer.clone(),
                    empties.recent_files.clone(),
                ...
                WidgetPayload::VideoPlayer(v) => (
                    ...
                    empties.media.clone(),
                    empties.audio_player.clone(),
                    build_video_player_model(v, &self.locale),
                    empties.password.clone(),
                    empties.viewer.clone(),
                    empties.recent_files.clone(),
                    empties.file_manager.clone(),
        ...
        WidgetFrameModel {
            ...
            search: search_model,
            media: media_model,
            audio_player: audio_player_model,
            video_player: video_player_model,
...
/// Pre-built empty widget sub-models for every type. Frame construction fills
/// *all* sibling slots even for a clock widget; cloning a cached pack is a
/// handful of `SharedString` / `ModelRc` refcount bumps instead of ~650 Fluent
/// lookups per frame.
#[derive(Clone)]
struct EmptyWidgetPack {
    weather: WeatherModel,
    moon: MoonModel,
    jyotish: JyotishModel,
    clock: ClockModel,
    system: SystemModel,
    ...
    search: SearchModel,
    media: MediaModel,
    audio_player: AudioPlayerModel,
    video_player: VideoPlayerModel,
...
impl EmptyWidgetPack {
    fn build(locale: &LocaleManager) -> Self {
        Self {
            weather: empty_weather_model(locale),
            moon: empty_moon_model(locale),
            jyotish: empty_jyotish_model(locale),
            clock: empty_clock_model(locale),
            system: empty_system_model(locale),
            processes: empty_processes_model(locale),
            calculator: empty_calculator_model(locale),
            notes: empty_notes_model(locale),
            calendar: empty_calendar_model(locale),
            rss: empty_rss_model(locale),
            search: empty_search_model(locale),
            media: empty_media_model(locale),
            audio_player: empty_audio_player_model(locale),
            video_player: empty_video_player_model(locale),
            password: empty_password_model(locale),
            viewer: empty_viewer_model(locale),
    ...
    fn cached(locale: &LocaleManager) -> Self {
        use std::cell::RefCell;
        thread_local! {
            static CACHE: RefCell<Option<(String, EmptyWidgetPack)>> =
                const { RefCell::new(None) };
        }
        let tag = locale.current().as_str().to_owned();
        CACHE.with(|slot| {
            let mut guard = slot.borrow_mut();
            if let Some((cached_tag, pack)) = guard.as_ref() {
                if *cached_tag == tag {
                    return pack.clone();
                }
            }
            let pack = Self::build(locale);
            *guard = Some((tag, pack.clone()));
            pack
        })
    }
}
...
fn default_frame_data_extended(
    ...
    (
        ...
        empties.search,
        empties.media,
        empties.audio_player,
        empties.video_player,
        empties.password,
        empties.viewer,
