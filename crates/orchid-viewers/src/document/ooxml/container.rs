...
pub fn open_document(path: &Path) -> Result<Document> {
    ...
    let header_first = load_story_part(
        &package,
        &rels,
        page_setup.header_first_r_id.as_deref(),
        "hdr",
        &styles,
        &numbering,
    )?;
    let footer_first = load_story_part(
        &package,
        &rels,
        page_setup.footer_first_r_id.as_deref(),
        "ftr",
        &styles,
        &numbering,
    )?;
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  <Relationship Id="rId8" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/footer" Target="footer1.xml"/>
    ...
    #[test]
    fn save_and_reopen_first_page_header_footer_stories() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("hf-first.docx");
        let file = File::create(&path).unwrap();
        let mut zip = ZipWriter::new(file);
        let opts = SimpleFileOptions::default();
        zip.start_file("[Content_Types].xml", opts).unwrap();
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  <Relationship Id="rId10" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/footer" Target="footer2.xml"/>
        ...
        zip.write_all(
            br#"<?xml version="1.0"?>
        <w:hdr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
          <w:p><w:r><w:t>DefaultHdr</w:t></w:r></w:p>
        </w:hdr>"#,
        )
        .unwrap();
        ...
        zip.write_all(
            br#"<?xml version="1.0"?>
        <w:hdr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
          <w:p><w:r><w:t>FirstHdr</w:t></w:r></w:p>
        </w:hdr>"#,
        )
        .unwrap();
        zip.start_file("word/footer1.xml", opts).unwrap();
        zip.write_all(
            br#"<?xml version="1.0"?>
        <w:ftr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
          <w:p><w:r><w:t>DefaultFtr</w:t></w:r></w:p>
        </w:ftr>"#,
        )
        .unwrap();
        ...
        zip.write_all(
            br#"<?xml version="1.0"?>
        <w:ftr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
          <w:p><w:r><w:t>FirstFtr</w:t></w:r></w:p>
        </w:ftr>"#,
        )
        .unwrap();
        zip.finish().unwrap();
        ...
        assert_eq!(doc.header[0].plain_text(), "DefaultHdr");
        assert_eq!(doc.header_first[0].plain_text(), "FirstHdr");
        assert_eq!(doc.footer[0].plain_text(), "DefaultFtr");
        assert_eq!(doc.footer_first[0].plain_text(), "FirstFtr");

        doc.header_first[0].runs[0].text = "NewFirst".into();
        save_document_sync(&doc, &path).unwrap();
        let loaded = open_document(&path).unwrap();
