...
- **Command system** (`command`) — a registry keyed by stable command ids, a shell-like command parser (`orc widget create …`), a fuzzy-search palette over the registry, and a keyboard shortcut type with canonical parsing and reserved-shortcut detection. File-manager `fs.*` ids are remappable actions, not `orc` verbs — see [`docs/commands.md`](../../docs/commands.md).
