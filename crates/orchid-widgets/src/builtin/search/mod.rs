...
pub use sources::{
    ActionTarget, CalculatorSource, CalendarSource, CommandsSource, FilesSource, JyotishSource,
    SearchCandidate, SearchSource, SettingsSource,
};
...
impl Widget for UniversalSearchWidget {
    ...
    async fn on_activate(&mut self, _ctx: &WidgetContext) -> WidgetResult<()> {
        if self.inner.aggregator.is_none() {
            *self.inner.error.write() = Some("search-sources-unconfigured".into());
        } else {
