...
impl<'a> Parser<'a> {
    ...
    fn parse_add(&mut self) -> Result<f64, CalcError> {
        ...
        loop {
            ...
            match self.peek() {
                Some(b'+') => {
                    self.bump();
                    left = finite(left + self.parse_mul()?)?;
                }
                Some(b'-') => {
                    self.bump();
                    left = finite(left - self.parse_mul()?)?;
    ...
    fn parse_mul(&mut self) -> Result<f64, CalcError> {
        ...
        loop {
            ...
            match self.peek() {
                Some(b'*') => {
                    self.bump();
                    left = finite(left * self.parse_power()?)?;
                }
                Some(b'/') => {
                    self.bump();
    ...
    fn parse_number(&mut self) -> Result<f64, CalcError> {
        ...
        let s =
            std::str::from_utf8(&self.src[start..self.i]).map_err(|_| CalcError::InvalidInput)?;
        s.parse::<f64>()
...
fn apply_fn(name: &str, x: f64, angle: AngleMode) -> Result<f64, CalcError> {
    let r = match name {
        ...
        "asin" => angle.radians_to(x.asin()),
        "acos" => angle.radians_to(x.acos()),
        "atan" => angle.radians_to(x.atan()),
        "sinh" => x.sinh(),
        "cosh" => x.cosh(),
        "tanh" => x.tanh(),
        "log" | "log10" => {
...
mod tests {
    ...
    fn basic_ops() {
        assert_eq!(
            evaluate_expression("2+3*4", AngleMode::Degrees).unwrap(),
            14.0
        );
        assert_eq!(
            evaluate_expression("(2+3)*4", AngleMode::Degrees).unwrap(),
    ...
    fn divide_by_zero() {
        assert_eq!(
            evaluate_expression("1/0", AngleMode::Degrees),
